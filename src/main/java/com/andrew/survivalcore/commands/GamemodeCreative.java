        ...
        if (!cooldown.asMap().containsKey(player.getUniqueId())) {
            ...
            if (args.length != 0) {
                ...
                if (player != target) {
                    ...
                    player.sendMessage(ChatColorUtil.colorize("&aYou have set " + target.getName() + "'s gamemode to creative."));
                    target.sendMessage(ChatColorUtil.colorize("&a" + player.getName() + " has set your gamemode to creative."));
                    return;
                }
            }
            player.setGameMode(org.bukkit.GameMode.CREATIVE);
            player.sendMessage(ChatColorUtil.colorize("&aYou have set your gamemode to creative."));
        } else {
